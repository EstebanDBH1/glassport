...
let hora = fecha.toLocaleTimeString("es-CO", opciones);
let ampm = hora.split(" ")[1];
hora = hora.split(" ")[0];
hora = hora.split(":")[0];
hora = hora % 12;
hora = hora ? hora : 12;
...
let h = (document.querySelector(".hora").innerHTML = `${horaActual}`);
...
let h1Reveal = gsap.from(".title-front-about ul li span", {
  y: "100%",
...
let wrapperImg = gsap.to(".wrapper-img", {
  y: "100%",
...
ScrollTrigger.create({
  trigger: ".title-front-about",
  ToggleActions: "none none none playground",
  start: "top bottom",
  end: "bottom  center",
  animation: h1Reveal,
  duration: 0.1
...
let reveal = gsap.to(".col-right p", {
...
tlm.from(".col-left-habi p span", 1, {
  y: 50,
  ease: Power3.easeOut,
  duration: 2,
  stagger: 0.05,
